...
CarCollatedTrajectoryBuilder::CarCollatedTrajectoryBuilder(
...
const Submaps* CarCollatedTrajectoryBuilder::submaps() const {
  return wrapped_trajectory_builder_->submaps();
}

const TrajectoryBuilder::PoseEstimate&
CarCollatedTrajectoryBuilder::pose_estimate() const {
  return wrapped_trajectory_builder_->pose_estimate();
}

void CarCollatedTrajectoryBuilder::AddSensorData(
...
void CarCollatedTrajectoryBuilder::HandleCollatedSensorData(
    const string& sensor_id, std::unique_ptr<sensor::Data> data) {
...
void CarCollatedTrajectoryBuilder::SetPose(const transform::Rigid3d& pos)
{
  wrapped_trajectory_builder_->SetPose(pos);
}

const void CarCollatedTrajectoryBuilder::GetPose(transform::Rigid3d& pos, double& confidence) const
{
  wrapped_trajectory_builder_->GetPose(pos, confidence);
}
}  // namespace mapping
}  // namespace cartographer
