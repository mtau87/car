...
#ifndef CARTOGRAPHER_MAPPING_2D_CAR_LOCALIZATION_H_
#define CARTOGRAPHER_MAPPING_2D_CAR_LOCALIZATION_H_
...
class CarLocalization {
 public:
  ...
  explicit CarLocalization(
      const proto::LocalTrajectoryBuilderOptions& options,
      const ProbabilityGrid& probability_grid);
  ~CarLocalization();

  CarLocalization(const CarLocalization&) = delete;
  CarLocalization& operator=(const CarLocalization&) = delete;

  const mapping::GlobalTrajectoryBuilderInterface::PoseEstimate& pose_estimate()
      const;
  void AddHorizontalLaserFan(
...
#endif  // CARTOGRAPHER_MAPPING_2D_CAR_LOCALIZATION_H_