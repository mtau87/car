...
proto::LocalTrajectoryBuilderOptions CreateCarLocalizationOptions(
...
sensor::LaserFan CarLocalization::TransformAndFilterLaserFan(
...
void CarLocalization::ScanMatch(
  ...
  if (options_.use_online_correlative_scan_matching()) {
    confidence = real_time_correlative_scan_matcher_.Match(
        pose_prediction_2d, filtered_point_cloud_in_tracking_2d,
        probability_grid_, &initial_ceres_pose);
  }
  ...
  ceres_scan_matcher_.Match(pose_prediction_2d, initial_ceres_pose,
                            ...
                            probability_grid_, &tracking_2d_to_map,
...
void
CarLocalization::AddHorizontalLaserFan(
...
CarLocalization::pose_estimate() const {
  return last_pose_estimate_;
}

void CarLocalization::AddImuData(
...
void CarLocalization::AddOdometerData(
...
void CarLocalization::InitializeImuTracker(const common::Time time) {
  if (imu_tracker_ == nullptr) {
...
void CarLocalization::Predict(const common::Time time) {
