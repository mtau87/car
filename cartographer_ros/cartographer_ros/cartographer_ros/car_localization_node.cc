...
LocNode::LocNode(const NodeOptions& options, tf2_ros::Buffer* const tf_buffer, const cartographer::mapping_2d::ProbabilityGrid& probability_grid)
    : options_(options), map_builder_bridge_(options_, tf_buffer, probability_grid), terminating_(false) {
...
void LocNode::Initialize() {
  ...
  if (options_.map_builder_options.use_trajectory_builder_2d()) {
    ...
    occupancy_grid_thread_ =
        std::thread(&LocNode::SpinOccupancyGridThreadForever, this);
  }

  if(options_.provide_pose)
  {
 	  pose_publisher_ = node_handle_.advertise<::geometry_msgs::PoseStamped>( kPoseTopic, kLatestOnlyPublisherQueueSize, true);  	
  }
  ...
  wall_timers_.push_back(node_handle_.createWallTimer(
      ...
      &LocNode::PublishTrajectoryStates, this));
}

::ros::NodeHandle* LocNode::node_handle() { return &node_handle_; }

CarLocalizationBridge* LocNode::map_builder_bridge() { return &map_builder_bridge_; }

void LocNode::PublishTrajectoryStates(const ::ros::WallTimerEvent& timer_event) {
  carto::common::MutexLocker lock(&mutex_);
      ...
      if (options_.provide_pose)
	  	{
  		  ...
  		  pose.pose = ToGeometryMsgPose(tracking_to_map);
...
void LocNode::SpinOccupancyGridThreadForever() {
  for (;;) {
